...
<article id="post-<?php the_ID(); ?>" <?php post_class('post-wrap '. $sticky_class); ?>  >
	<div class="content">
		<?php if( ( has_post_format('audio') || has_post_format('gallery') || has_post_format('video' ) || has_post_thumbnail() ) && transflash_blog_show_media() == 'yes' ) : ?>
		...
		<?php if( transflash_blog_show_title() == 'yes' ){
			get_template_part( 'template-parts/parts/title' );
		} ?>
		
		<?php get_template_part( 'template-parts/parts/meta' ); ?>
		
		<?php if( transflash_blog_show_excerpt() == 'yes' ){ ?>
			<div class="post-excerpt">
				<p>
					<?php echo transflash_custom_text( get_the_excerpt(), 14 ); ?>
				</p>
			</div>
		<?php } ?>

		<?php if( transflash_blog_show_readmore() == 'yes' ){
