...
if( !function_exists( 'transflash_get_current_id' )):
	function transflash_get_current_id(){
...
if (!function_exists('transflash_is_elementor_active')) {
    function transflash_is_elementor_active(){
        return did_action( 'elementor/loaded' );
    }
}

if (!function_exists('transflash_is_woo_active')) {
    function transflash_is_woo_active(){
        return class_exists('woocommerce');    
    }
}

if (!function_exists('transflash_is_blog_archive')) {
    function transflash_is_blog_archive() {
...
function transflash_get_id_by_slug( $page_slug ) {
...
function transflash_custom_text ($content = "",$limit = 15) {
 ...
 */
if ( ! function_exists( 'transflash_google_font_sanitization' ) ) {
    function transflash_google_font_sanitization( $input ) {
...
if ( ! function_exists( 'transflash_default_primary_font' ) ) {
    function transflash_default_primary_font() {
...
if ( ! function_exists( 'transflash_default_second_font' ) ) {
    function transflash_default_second_font() {
        $customizer_defaults = json_encode(
            array(
                'font' => 'Raleway',
                'regularweight' => '100,200,300,400,500,600,700,800,900',
...
if ( ! function_exists( 'transflash_woo_sidebar' ) ) {
    function transflash_woo_sidebar(){
...
if( !function_exists( 'transflash_blog_show_readmore' ) ){
    function transflash_blog_show_readmore(){
...
if( !function_exists( 'transflash_post_show_media' ) ){
    function transflash_post_show_media(){
        $show_media = get_theme_mod( 'blog_single_show_media', 'yes' );
        return isset( $_GET['show_media'] ) ? $_GET['show_media'] : $show_media;
    }
}

if( !function_exists( 'transflash_post_show_title' ) ){
    function transflash_post_show_title(){
        $show_title = get_theme_mod( 'blog_single_show_title', 'yes' );
        return isset( $_GET['show_title'] ) ? $_GET['show_title'] : $show_title;
    }
}

if( !function_exists( 'transflash_post_show_date' ) ){
    function transflash_post_show_date(){
        $show_date = get_theme_mod( 'blog_single_show_date', 'yes' );
        return isset( $_GET['show_date'] ) ? $_GET['show_date'] : $show_date;
    }
}

if( !function_exists( 'transflash_post_show_cat' ) ){
    function transflash_post_show_cat(){
        $show_cat = get_theme_mod( 'blog_single_show_cat', 'yes' );
        return isset( $_GET['show_cat'] ) ? $_GET['show_cat'] : $show_cat;
    }
}

if( !function_exists( 'transflash_post_show_author' ) ){
    function transflash_post_show_author(){
        $show_author = get_theme_mod( 'blog_single_show_author', 'yes' );
        return isset( $_GET['show_author'] ) ? $_GET['show_author'] : $show_author;
    }
}

if( !function_exists( 'transflash_post_show_comment' ) ){
    function transflash_post_show_comment(){
        $show_comment = get_theme_mod( 'blog_single_show_comment', 'yes' );
        return isset( $_GET['show_comment'] ) ? $_GET['show_comment'] : $show_comment;
    }
}

if( !function_exists( 'transflash_post_show_tag' ) ){
    function transflash_post_show_tag(){
